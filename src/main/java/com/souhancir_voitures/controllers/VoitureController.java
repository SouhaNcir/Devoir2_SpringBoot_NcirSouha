package com.souhancir_voitures.controllers;
...
public class VoitureController {
	...
	public String listeVoitures(ModelMap modelMap,
	    ...
	    modelMap.addAttribute("pages", new int[voits.getTotalPages()]);  // ✅ "pages" avec un s
	    modelMap.addAttribute("currentPage", page);
	    modelMap.addAttribute("size", size);  // ✅ Ajout du size
			...
			ModelMap modelMap) throws ParseException
	{
		...
		Date dateFabr = dateformat.parse(String.valueOf(date));
		voiture.setDateFabrication(dateFabr);
		
		Voiture saveVoiture = voitureService.saveProduit(voiture);
	...
	@RequestParam (name="size", defaultValue = "4") int size){
		voitureService.deleteProduitById(id);
		Page<Voiture> voits = voitureService.getAllVoituresParPage(page, size);
	    modelMap.addAttribute("voitures", voits);
	    modelMap.addAttribute("pages", new int[voits.getTotalPages()]);  // ✅ "pages" avec un s
	    modelMap.addAttribute("currentPage", page);
